...
export function AssessmentPanel({
  ...
  run: AssessmentRun;
  /** Ratification standing (load.ts ratification()). */
  standing: {
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <p
            className={`font-mono text-[10px] uppercase tracking-[0.14em] ${
              standing?.status === "contested" ? "text-ochre" : "text-copper"
            }`}
          >
            {standing?.status === "ratified"
              ? `ratified by independent models · ${standing.checksDate}`
              : standing?.status === "contested"
                ? "contested by independent models"
